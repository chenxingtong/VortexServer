...
WebServer::WebServer(int port, int trigMode, int timeoutMs, bool optLinger, int sqlPort,
                     ...
                     int threadNum, int maxRequests, bool openLog, int logLevel, int logQueSize) :
    port(port),
    openLinger(optLinger), timeoutMs(timeoutMs), shutdown(false), timer(new HeapTimer()),
    epoller(new Epoller())
{
  //内部调用malloc动态分配缓存
...
void WebServer::initEventMode(int trigMode)
{
  ...
  {
  case 0: break;
  case 1: connEvent |= EPOLLET; break;
  case 2: listenEvent |= EPOLLET; break;
  case 3:
    listenEvent |= EPOLLET;
    connEvent |= EPOLLET;
    break;
...
void WebServer::start()
{
  ...
  {
    //返回下一个计时器超时的时间
    if (timeoutMs > 0) timeMs = timer->getNextTick();
