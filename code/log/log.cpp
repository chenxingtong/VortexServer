...
void Log::write(int level, const char* format, ...)
{
  ...
  if (today != t.tm_mday || (lineCount && (lineCount % MAX_LINES) == 0))
  {
    ...
    {
      snprintf(
          newFile, LOG_NAME_LEN - 72, "%s/%s-%d%s", path_, tail, (lineCount / MAX_LINES), suffix_);
    }
  ...
  //向文件中互斥写日志
  {
    unique_lock<mutex> locker(mtx);
    lineCount++;
    int n = snprintf(buffer.beginWrite(),
                     128,
...
void Log::appendLogLevelTitle(int level)
{
  switch (level)
  {
  case 0: buffer.append("[debug]: ", 9); break;
  case 1: buffer.append("[info] : ", 9); break;
  case 2: buffer.append("[warn] : ", 9); break;
  case 3: buffer.append("[error]: ", 9); break;
  default: buffer.append("[info] : ", 9); break;
  }
}
